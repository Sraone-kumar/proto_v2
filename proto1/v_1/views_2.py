...
def get_info(request):
    ...
    if request.GET.get('action') == 'info' or request.GET.get('action') == 'fac_sort_info':
        ...
        department = list(Branch_table.objects.filter(
            branch_id=branch).values('department_id'))
        faculty = list(faculty_table.objects.filter(Department_id_id=department[0].get(
            'department_id')).values('id', 'faculty_name', 'No_hrs_per_week'))
        ...
        courses = list(subjects_table.objects.filter(department_id_id=department[0].get(
            'department_id'), semester_taught_id=semester).values('subject_id', 'subject_name'))
        check_query = sub_fac_relation.objects.filter(
            branch_id_id=branch, section_id=section, semester_id=semester)
        ...
        if check_query.exists():
            ...
            return JsonResponse({"faculty": faculty, "subjects": courses})
...
def get_info_back(request):
    if(request.GET.get('action') == 'get_fac_relation'):
        ...
        tasks = sub_fac_relation.objects.filter(
            branch_id_id=branch, section_id=section, semester_id=semester_get)
        classquery = class_time_table.objects.filter(
            branch_id=branch, section=section, semester=semester_get)
        department_id = list(Branch_table.objects.filter(
            branch_id=branch).values('department_id'))
        labTimeTable = lab_time_table.objects.filter(
            branch_id=branch, semester_id=semester_get, section_id=section)
       # print("department_id:",department_id[0]['department_id'])
        labQuery = list(lab_information_table.objects.filter(
            lab_department_id=department_id[0]['department_id']).values('lab_id', 'lab_name'))
    ...
    if request.GET.get('action') == 'display_lab_info':
        ...
        get_branch = Branch_table.objects.filter(
            department_id=dep).values('branch_id')
        query = lab_time_table.objects.filter(
            branch_id=get_branch[0].get('branch_id'), lab_id=lab)
...
def total_script(request):
    ...
    time = [(x+1, k) for x, k in enumerate(time)]
    weekdays = list(Week_table.objects.all().values())
    weekdays = [(x+1, k) for x, k in enumerate(weekdays)]
